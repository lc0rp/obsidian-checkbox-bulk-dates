...
export default class CheckboxBulkDatePlugin extends Plugin {
	...
	async onload() {
		...
		this.addCommand({
			...
			editorCallback: (editor: Editor, view: MarkdownView) => {
				...
				if (addedCount > 0) {
					...
					new Notice(
						`Added ${addedCount} created date${
							addedCount > 1 ? "s" : ""
						} to current file`
					);
					this.debug(
						`Added ${addedCount} creation dates to current file`
					);
				} else {
					new Notice(
						"No unchecked checkboxes without creation dates found"
					);
				}
			},
		});
		...
		this.addCommand({
			...
			callback: async () => {
				...
				const initialNotice = new Notice(
				...
				try {
					...
					for (let i = 0; i < files.length; i += batchSize) {
						...
						if (files.length > 20) {
							initialNotice.setMessage(
								`Processed ${processedFiles}/${files.length} files... (${totalCount} dates added)`
							);
						}
					...
					initialNotice.hide();
					new Notice(
						`Added ${totalCount} created date${
							totalCount > 1 ? "s" : ""
						} across ${processedFiles} files`
					);
					this.debug(
						`Added ${totalCount} creation dates across ${processedFiles} files`
					);
				} catch (error) {
					initialNotice.hide();
					console.error("Error during vault-wide processing:", error);
					new Notice(
						"Error occurred while processing vault. Check console for details."
					);
				}
			},
		});
	...
	private addCreatedToText(text: string, file: TFile | null = null): string {
		...
		const result = text.replace(
			...
			(_match, taskPrefix, rest) =>
				`${taskPrefix} ${rest} ${CREATED} ${dateToUse}`
		);
		return result;
	}
...
class CheckboxBulkDateSettingTab extends PluginSettingTab {
	...
	display(): void {
		...
		containerEl.createEl("h2", {
			text: "Checkbox Bulk Creation Date Settings",
		});

		new Setting(containerEl)
			.setName("Enable real-time adding")
			.setDesc(
				"Automatically add creation dates when creating new checkboxes"
			)
			.addToggle((toggle) =>
				toggle
		...
		new Setting(containerEl)
			.setName("Date source")
			.setDesc(
				"Choose whether to use file creation date or file modified date for retroactive adding"
			)
			.addDropdown((dropdown) =>
				dropdown
					...
					.setValue(
						this.plugin.settings.useFileCreationDate
							? "creation"
							: "modified"
					)
					.onChange(async (value) => {
		...
		new Setting(containerEl)
			...
			.setDesc(
				"Enable detailed console logging for troubleshooting (requires Developer Tools)"
			)
			.addToggle((toggle) =>
				toggle
